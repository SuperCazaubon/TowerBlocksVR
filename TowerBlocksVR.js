...
<head>
  <meta charset="UTF-8">
  <title>Tower Blocks VR (3D HUD + Découpe)  (IndexV9)</title>
  ...
  <script type="module">
    ...
    class HUD {
      constructor(scene) {
        ...
        loader.load("./cdn.jsdelivr.net/npm/three@0.151.3/examples/fonts/helvetiker_regular.typeface.json", (font) => {
          ...
          this.scoreMesh.position.set(-8, 18, -15);
          scene.add(this.scoreMesh);

          this.instructionsMesh = this.makeText("Press SPACE or Click", 1, 0x00ffcc);
          this.instructionsMesh.position.set(-6, 12, -15);
          scene.add(this.instructionsMesh);

          this.gameOverMesh = this.makeText("GAME OVER", 2, 0xff3333);
          this.gameOverMesh.position.set(-8, 14, -20);
          this.gameOverMesh.visible = false;
          scene.add(this.gameOverMesh);

          this.startMesh = this.makeText("START", 1.5, 0x33ff33);
          this.startMesh.position.set(-5, 10, -15);
      ...
      setScore(score) {
        this.updateText(this.scoreMesh, `Score ${score}`, 1, 0xffff00);
      }
      ...
      setStartText(text) { this.updateText(this.startMesh, text, 1.5, 0x33ff33); }
    }
    ...
    class Game {
      ...
      addBlock() {
		...
		this.stage.camera.position.set(this.stage.camera.position.x, this.stage.camera.position.y+1, this.stage.camera.position.z);																															   
      }
